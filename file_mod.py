...
@file_manager_bp.route('/git_sync', methods=['POST'])
def git_sync():
    """Two-way sync: Environment-tailored execution for Windows vs PythonAnywhere."""
    if not session.get('is_admin'):
        return jsonify({"output": "ACCESS_DENIED"}), 403
    ...
    if is_windows:
        ...
        output_log += "[System] Local Windows environment detected. Using standard tracking branch...\n"
        
        # Add and commit local changes first so working tree is clean
        run_git_cmd(['add', '.'])
        run_git_cmd(['commit', '-m', 'Auto-sync update'])
        
        # Pull and push using standard origin tracking
